...
census_combined <- left_join(specific_only, general_only) %>% 
  ...
  filter(!specific_region %in% c("Mountain North", "Mountain South"))

census_combined <- census_combined %>%
  mutate(general_region = ifelse(is.na(general_region), "West", general_region))


write.csv(census_combined, file = here::here("data_intermediate/census_data.csv"))
...
census_sun <- census_combined %>% 
  filter(indicator == "any_insecurity") %>% 
  group_by(general_region) %>% 
  summarize(total_homes = sum(total_us)) 

census_spec <- census_combined %>% 
  filter(indicator == "any_insecurity") %>% 
  select(total_us, specific_region, general_region)
...
ggplot(data=census_any, aes(x=fct_reorder(specific_region,general_region), y=pct, fill=general_region))+
  geom_bar(stat="identity") +
  geom_text(aes(label=scales::percent(pct, accuracy = 1), y=label_position), size = 6, color="white", face= "bold")+
  labs(x="", y="", 
       title="Percent of Homes Experiencing Energy Insecurity (2020)",
       size = "",
       subtitle = "By Census Region, General & Specific",
  ...
  aes(x = fct_reorder(specific_region, general_region), y = pct, label = specific_region),
  position = position_stack(vjust = 1.35), # Adjust 'vjust' for label positioning
  size = 6,
